...
export default function AgendaMedica() {
  ...
  const buscarConsultasDoBanco = async () => {
    try {
      setCarregando(true);
      // 🌐 Buscando direto da mesma API PHP que a secretaria usa
      const response = await fetch('http://localhost/saude-digital-api/get_estatisticas.php');
      const data = await response.json();
      
      if (data && data.agenda) {
        // 🔍 Filtra dinamicamente os agendamentos que pertencem a este médico específico
        // Evita erros comparando tanto pelo Nome quanto por ID se estiver disponível
        const minhasConsultas = data.agenda.filter(item => 
          item.medico?.toLowerCase().includes(medicoLogado?.nome?.toLowerCase()) ||
          item.medico_id === medicoLogado?.id
        );
        setConsultas(minhasConsultas);
      }
    } catch (err) {
      console.error("Erro ao carregar agenda do banco:", err);
                ...
                <div key={item.id} className="p-8 flex items-center justify-between hover:bg-gray-50/30 transition-colors">
                  <div className="flex items-center gap-6">
                    <span className="font-black text-lg text-indigo-600 bg-indigo-50 px-4 py-2.5 rounded-2xl flex items-center gap-2">
                      <Clock size={16} /> {item.hora}
                    </span>
                    <div>
                      <p className="font-black text-slate-800 text-lg">{item.paciente}</p>
                  ...
                  <button 
                    onClick={() => alert(`Iniciando Prontuário Eletrônico para: ${item.paciente}`)}
